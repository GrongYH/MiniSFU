...
func TestSFU_SessionScenarios(t *testing.T) {
	log.Init("debug")
	sfu := NewSFU(Config{IceLite: true})
				...
				for _, action := range step.actions {
					func() {
						...
						case "join":
							...
							pub, err := api.NewPeerConnection(webrtc.Configuration{SDPSemantics: webrtc.SDPSemanticsUnifiedPlan})
							assert.NoError(t, err)
							sub, err := api.NewPeerConnection(webrtc.Configuration{SDPSemantics: webrtc.SDPSemanticsUnifiedPlan})
							assert.NoError(t, err)
							local := NewPeer(sfu)
							_, err = pub.CreateDataChannel("mini-sfu", nil)
							...
							p.local.OnIceCandidate = func(init *webrtc.ICECandidateInit, i int) {
								switch i {
								case subscriber:
									log.Debugf("p.local.OnIceCandidate subscriber success")
									log.Debugf("%v", init.Candidate)
							...
							p.local.OnOffer = func(o *webrtc.SessionDescription) {
								...
								assert.NoError(t, err)
								gatherComplete := webrtc.GatheringCompletePromise(p.remoteSub)
